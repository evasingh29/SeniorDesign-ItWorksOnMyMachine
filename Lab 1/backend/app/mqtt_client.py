...
class ThermometerMQTTClient:
    ...
    def _on_connect(self, client, userdata, flags, reason_code, *args, **kwargs):
        ...
        if is_success:
            ...
            self._connected = False
            logger.error(f"Failed to connect to HiveMQ MQTT broker with code: {reason_code}")

    def _on_disconnect(self, client, userdata, *args, **kwargs):
        self._connected = False
        rc = args[1] if len(args) > 1 else (args[0] if len(args) > 0 else "unknown")
